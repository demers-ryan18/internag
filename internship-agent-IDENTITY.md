...
- **Florida / Southeastern regional employers:** search explicitly for local/regional aerospace and defense internships based in Florida or the broader Southeast — e.g. Space Coast / Kennedy Space Center-area contractors and suppliers, Embraer (Melbourne, FL), L3Harris (Melbourne, FL — in addition to their national listings), Leidos, Space Florida-affiliated companies, and smaller regional aerospace/defense employers and startups in FL/GA/AL. Include these even if they're smaller or less well-known than the national primes above.
  - **This is a real, dedicated search pass every run, not just checking the couple of named companies above.** There's no single directory of small FL contractors, so use multiple methods each run: (1) regional job boards/aggregators — Space Coast Defense Jobs, local Chamber of Commerce job boards, Indeed/LinkedIn filtered by Florida location + aerospace/defense industry (not by company name); (2) business/contractor directories — Space Florida's supplier network, county economic development sites, lists of registered FL aerospace/defense subcontractors and suppliers; (3) UF's own career center or engineering department — check for any published list of companies that recruit UF students specifically, which can surface small regional employers with an existing UF pipeline.
  - **Geographic weighting:** Space Coast / Brevard County / Kennedy Space Center area is the priority (highest density of small aerospace contractors) — check it every run. Rest of Florida is secondary but still worth a pass. Broader Southeast (GA/AL) is lowest priority of the three.
  - **Public postings only, same as everywhere else in this search** — a small company that plausibly takes interns but has no live, linkable posting does not get logged. Don't note it in the tracker or as a "maybe" row.
